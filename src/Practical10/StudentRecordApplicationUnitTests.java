...
public class StudentRecordApplicationUnitTests {
	...
	public void testPerformSortOperation(String itemToSort) {
		...
		when(suMock.sortOnName(ArgumentMatchers.anyList())).thenReturn(listToUse);	
		when(suMock.sortOnTotalMark(ArgumentMatchers.anyList())).thenReturn(listToUse);	
		when(suMock.sortOnSubject(ArgumentMatchers.anyList(),anyString())).thenReturn(listToUse);	
		
		sra.performSortOperation(itemToSort);
		
		if (itemToSort.equals("name")) {
			verify(suMock, never()).sortOnTotalMark(anyList());
			verify(suMock, never()).sortOnSubject(anyList(), anyString());
		}
		else if (itemToSort.equals("total")) {
			verify(suMock, never()).sortOnName(anyList());
			verify(suMock, never()).sortOnSubject(anyList(), anyString());
		}
		else {
			verify(suMock, never()).sortOnName(anyList());
	...
	public void testPerformSortOperationError2(String itemToSort) {
		...
		// setup so that all the methods when called will return the 
		// Student ArrayList we set up earlier
		when(suMock.sortOnName(anyList())).thenReturn(listToUse);	
		when(suMock.sortOnTotalMark(anyList())).thenReturn(listToUse);	
		when(suMock.sortOnSubject(anyList(),anyString())).thenReturn(listToUse);	
	...
	public void testperformFilterOperation(String filterString) {
		...
		when(suMock.filterOnSubjectNames(anyList(), eq(filterArgs))).thenReturn(listToUse);
	...
	public void testPerformFilterOperationError3() {
		...
		when(suMock.filterOnSubjectNames(anyList(), any(String[].class))).
