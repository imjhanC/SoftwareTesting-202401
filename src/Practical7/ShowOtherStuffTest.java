...
public class ShowOtherStuffTest {
	...
	public void testNumTimesCalled() {
		...
		verify(msMock, times(2)).secondMethod(anyString());
		
		verify(msMock, atLeastOnce()).secondMethod("Great");//fail if the method is not been called with this parameter
		verify(msMock, atLeast(3)).secondMethod("cat");//fail if called less than 3 times with this parameter
		verify(msMock, atMost(2)).secondMethod("dog");//fail if call more than 2 times with this parameter
		verify(msMock, never()).secondMethod("bird");//fail if call is made with this parameter
		verify(msMock, never()).thirdMethod(anyString());//fail if call is made with any string values

		verifyNoInteractions(rsMock);//verify no method invoked on this test double object
	}
